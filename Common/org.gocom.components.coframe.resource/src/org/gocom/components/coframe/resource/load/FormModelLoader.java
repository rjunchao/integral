...
import org.gocom.components.coframe.resource.load.register.FormRegister;
...
/**
 * 
 * 表单模型加载类，加载表单模型到缓存，并注册到资源中
 *
 * @author caozw (mailto:caozw@primeton.com)
 */
public class FormModelLoader implements IModelLoader {

	public boolean isSupportLazyLoading() {
		return false;
	}
	/**
	 * 载入表单资源时调用的方法，载入表单资源时向资源库注册可授权管控资源
	 */
	public void load(IResourceCollection modelResource) {
		...
		for (IModelResource resource : addedResources) {
			FormRegister.register(new File(resource.getPath()));
		}
		...
		for (IModelResource resource : deleteResources) {
			FormRegister.unRegister(new File(resource.getPath()));
		}
