...
<div id="layout1" class="nui-layout" style="width:100%;height:100%;">
	<div id="region1" region="west" title="业务领域管理" showHeader="true" class="sub-sidebar" width="240" allowResize="false">
		<ul id="tree1" class="nui-tree" style="width: 100%; height: 99%;"
					url="com.vbm.grc.basic.busiarea.busiarea.busiAreaTree.biz.ext"
					showTreeIcon="true" showTreeLines="true" textField="busi_name" idField="pk_busi_area"	
    ...
    <div title="center" region="center" style="border:0;padding-left:5px;padding-top:5px;">
    	<!--Tabs-->	
       <div id="busitabs" class="nui-tabs  bg-toolbar" activeIndex="0" style="width:100%;height:100%;">
		    <div name="tab_list" title="业务领域列表" url="<%=request.getContextPath() %>/grc/basic/busiarea/busiAreaList.jsp" visible="true" >
		    </div>
	   </div>
	</div>
</div>
...
<script type="text/javascript">
	 ...
	 //tabs里的页面JSP
	 var busi_info = {title:'业务领域信息',path:'<%=request.getContextPath() %>/grc/basic/busiarea/busiAreaInfo.jsp'};
	 var busit_list = {title:'业务领域列表',path:'<%=request.getContextPath() %>/grc/basic/busiarea/busiAreaList.jsp'};
	 //根据节点的类型，tab所load的页面
	 var busitabs_map = {};
	 //根节点
	 busitabs_map["root"] = [busit_list];
	 //业务领域下有子领域
	 busitabs_map["busigroup"] = [busi_info,busit_list];
	 //子领域
	 busitabs_map["busi"] = [busi_info];
	 
	 //设置URL的参数
	 //url: grc/system/busiarea/busiareaInfo.jsp
	...
	function refreshTab(node){
		//刷新右边tab
		var tabs = nui.get("busitabs");
		//根据节点类型，填充tab页面
		var busitabs = busitabs_map[node.type];
		//设置tab
		for(var i = 0; i < busitabs.length; i++){
			var obj = busitabs[i];
			//把节点信息传到右侧进行显示
			obj.url = setUrlParam(obj.path,node);
		}
		//设置右侧显示的JSP页面
		tabs.setTabs(busitabs);
	}	
	function refresh(){
		//添加元素后刷新
		//刷新树
