...
<!-- 这个div里是增删改查的按钮 -->
  		...
  		<table style="width:100%;">
  			<tr>
  				...
  				<td style="white-space:nowrap;">
  					<input id="key" class="nui-textbox" style="width:150px;" emptyText="控制执行等级编码或名称"/>
  ...
  <div id="datagrid" class="nui-datagrid" style="width:100%;height:90%;"
		...
		dataField="datas" onselectionchanged="selectionChanged" multiSelect="true">
		<div property="columns">
			<div field="pk_control_exec" type="checkcolumn" width="10px">选择</div>
			<div field="level_code" allowResize="true" width="25px" headerAlign="center">控制执行有效性编码</div>
			<div field="level_name" allowResize="true" width="25px" headerAlign="center">控制执行有效性名称</div>
     ...
     //在增加的时候：点击新增按钮进入新增的页面
       function addRow(){
         	...
         	//打开新增保存的页面
         	nui.open({
         		url: "<%=request.getContextPath() %>/grc/basic/controlexec/addControlExec.jsp",
         		title: "新增控制执行有效性",
         		width:600,
         		height:160,
      ...
      function editRow(){
         	...
         	if(row != null){
         	nui.open({
         		url: "<%=request.getContextPath() %>/grc/basic/controlexec/editControlExec.jsp",
         		title: "修改控制执行有效性",
         		width:600,
         		height:160,
         		onload:function(){
        ...
        if(rows.length > 0){
          ...
          nui.confirm("确定要删除吗？", "提示", function(action){
            if(action == "ok"){
              //确定删除
              nui.ajax({
                url: "com.vbm.grc.basic.control.exec.ControlExec.delControlExecVO.biz.ext",
                data: json,
                cache: false,
                type: "POST",
