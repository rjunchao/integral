...
<body>
	<div id="form1" method="post" class="nui-fit" style="height:60%;">
		<input id="pk_risk_incidence" name="pk_risk_incidence" class="nui-hidden" />
		<table style="width:100%;height:100%;table-layout:fixed;" class="nui-form-table_new">
			<tr>
				<td class="nui-form-label">概率等级编码：</td>
				<td>	
					<input id="incidence_code" name="incidence_code" class="nui-textbox nui-form-input" onvalidation="validateCode" required="true" vtype="maxLength:20"/>
				</td>
				<td class="nui-form-label">概率等级名称：</td>	
				<td>
					<input id="incidence_name" name="incidence_name" class="nui-textbox nui-form-input" onvalidation="validateName" required="true" vtype="maxLength:100"/>
				</td>
			</tr>
			<tr>
...
<script type="text/javascript">
    ...
    function validateCode(e){
		...
		//得到主键
		var pk = nui.get("pk_risk_incidence").getValue();
		var json = nui.encode({vo:{incidence_code:val,pk_risk_incidence:pk}});
		$.ajax({
    ...
    function saveData(){
    	...
    	form.validate();
		   ...
		   var json = nui.encode({vo:o});
			 nui.ajax({
			    url:"com.vbm.grc.basic.risk.incidence.riskincidence.editRiskIncidence.biz.ext",
			    type:'POST',
			    data:json,
			    cache:false,
			    contentType:'text/json',
