...
import com.grc.basic.vo.pub.BeanHelper;
import com.grc.basic.vo.pub.SuperVO;
import com.grc.common.jdbc.framework.mapping.IMappingMeta;
import com.grc.common.jdbc.framework.mapping.MappingMetaManager;
import com.grc.common.jdbc.framework.util.InOutUtil;
import com.vbm.grc.pubapp.converter.Converter;
import com.vbm.grc.pubapp.converter.EsaConverterManager;


public class ProcessorUtils {

	 private static Logger logger = TraceLoggerFactory.getLogger(ProcessorUtils.class);
	...
	private static List<?> toBeanListInner(ResultSet resultSet, Class<?> type,
		...
		//返回 方法数组，属性数组，sql类型数组，列的下标，converters
		MethodAndTypes methodAndTypes = getBeanInvokeAndTypes(type, resultSet,
				meta, columns);
	...
	public static MethodAndTypes getBeanInvokeAndTypes(Class<?> type,
		...
		int cols = metaData.getColumnCount();//得到查询的列数
		int len = columns == null ? 0 : columns.length;
		if (len == 0) {
			len = cols;
		}
		Method[] invokes = new Method[len];//方法对象数组
		Converter[] converters = new Converter[len];//
		String[] propNames = new String[len];//属性名数组
		int[] idxs = new int[len];//下标
		int[] types = new int[len];//返回值类型
		int j = 0;
		for (int i = 0; i < cols && j < len; i++) {
			int t = metaData.getColumnType(i + 1);//得到列的 SQL 类型。
			String propName = metaData.getColumnLabel(i + 1).toLowerCase();//指定列的建议标题。
			if (propName == null)
				propName = metaData.getColumnName(i + 1).toLowerCase();//得到指定列的名称
			if (columns != null)
		...
		Method[] temps = BeanHelper.getMethods(bean, propNames);//返回对象的get/set方法数组
		EsaConverterManager ecm = new EsaConverterManager();
		for (int i = 0; i < len; i++) {
			if (temps[i] == null) {//根据列的个数来遍历方法
				if (isSuperBean) {
	...
	private static Method getSuperBeanInvokeMethod(Object bean, String colName) {
		...
		if (pkFiledName.equals(colName)) {
			return BeanHelper.getMethod(bean, "primarykey");//返回得到主键的方法
		}
		return null;
