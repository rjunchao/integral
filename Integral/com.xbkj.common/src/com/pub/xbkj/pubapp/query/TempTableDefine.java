...
import com.pub.xbkj.pubapp.database.TempTable;
...
public class TempTableDefine {
  ...
  public String get(String[] ids) {
    ...
    if (length <= TempTableDefine.ONE_THRESHOLD) {
      tableName = this.get(ids, this.getTempTablePrefix() + "5H1");
    }
    else if (length <= TempTableDefine.TWO_THRESHOLD) {
      tableName = this.get(ids, this.getTempTablePrefix() + "1T1");
    }
    else if (length <= TempTableDefine.TRHREE_THRESHOLD) {
      tableName = this.get(ids, this.getTempTablePrefix() + "5T1");
    }
    else if (length <= TempTableDefine.FOUR_THRESHOLD) {
      tableName = this.get(ids, this.getTempTablePrefix() + "10T1");
    }
    else {
      tableName = this.get(ids, this.getTempTablePrefix() + "L1");
    }
  ...
  public String get(String[] id1, String[] id2) {
    ...
    tableName = dao.getTempTable(tableName, columns, columnTypes, types, data);
    return tableName;
  }
   ...
   * @return 三列主键临时表
   */
  public String get(String[] id1, String[] id2, String[] id3) {
    List<List<Object>> data = new ArrayList<List<Object>>();
   ...
   * @return 一列ID字段的临时表
   */
  public String getAnother(String[] ids) {
    String tableName = null;
    int length = ids.length;
    if (length <= TempTableDefine.ONE_THRESHOLD) {
      tableName = this.get(ids, this.getTempTablePrefix() + "5H1A");
    }
    else if (length <= TempTableDefine.TWO_THRESHOLD) {
      tableName = this.get(ids, this.getTempTablePrefix() + "1T1A");
    }
    else if (length <= TempTableDefine.TRHREE_THRESHOLD) {
      tableName = this.get(ids, this.getTempTablePrefix() + "5T1A");
    }
    else if (length <= TempTableDefine.FOUR_THRESHOLD) {
      tableName = this.get(ids, this.getTempTablePrefix() + "10T1A");
    }
    else {
      tableName = this.get(ids, this.getTempTablePrefix() + "L1A");
    }
