...
import com.xbkj.gd.integral.vos.OtherIntegralVO;
...
public class OtherIntegralBiz {
	 ...
	 */
	@Bizlet
	public OtherIntegralVO[] queryCustOtherIntegralDetail(Map<String,String> params,PageCond page){
		//判断是不是有参数
		String where = whereSql(params);
		//查询数据实体
		VOPageQuery<OtherIntegralVO> query = new VOPageQuery<OtherIntegralVO>(OtherIntegralVO.class);
		//查询的总记录
		String queryCountSql = " SELECT COUNT(1) FROM GD_OTHER_INTEGRAL_DETAIL T" +
								"	LEFT JOIN ORG_EMPLOYEE E ON E.`USERID`=T.`CREATE_USER`" +
								" 	LEFT JOIN ORG_ORGANIZATION O ON O.`ORGID` = T.`CREATE_USER_ORG`" +
								" WHERE INTEGRAL_TYPE='" + params.get("integral_type") + "'" + where;
	    //查询
		String querySql =" SELECT C.`CUSTOMER_NAME`, t.*," +
						"	E.`EMPNAME`, O.`ORGNAME` FROM GD_OTHER_INTEGRAL_DETAIL T" +
						"	LEFT JOIN ORG_EMPLOYEE E ON E.`USERID`=T.`CREATE_USER`" +
						" 	LEFT JOIN ORG_ORGANIZATION O ON O.`ORGID` = T.`CREATE_USER_ORG`" +
						" 	LEFT JOIN GD_CUSTOMER_INFO2 C ON T.`CUSTOMER_IDCARD` = C.`CUSTOMER_IDCARD` " +
						" WHERE INTEGRAL_TYPE='" + params.get("integral_type") + "'" + where;
		System.out.println("积分明细查询： " + querySql);
		OtherIntegralVO[] vos = query.query(querySql, queryCountSql, page);
		 if(vos != null && vos.length > 0){
			 if("Y".equals(params.get("hiddenFlag"))){
				 for(OtherIntegralVO vo : vos){
...
//		Map parameter = request.getParameterMap();
		...
		OtherIntegralVO[] integrals = this.queryCustOtherIntegralDetail(parameter, page);
		if(integrals == null){
			integrals = new OtherIntegralVO[0];
		}
		//导出
	//	System.out.println("export vo:" + integrals);
		
		HSSFWorkbook wb = new HSSFWorkbook();
		HSSFSheet sheet = wb.createSheet("积分明细表");
		...
		response.addHeader("Content-Disposition", "attachment;filename=" + 
						UserUtils.toUtf8String("积分明细表") + ".xls");
		try {
	 ...
	 */
	@Bizlet
	public MsgResponse addOtherIntegral(OtherIntegralVO vo){
		MsgResponse vipIntegral = addIntegral(vo);
		System.out.println("积分操作成功");
		return vipIntegral;
	}
	
	public MsgResponse addIntegral(OtherIntegralVO vo){
		...
		try {
			...
			GdDataHandlerUtils<OtherIntegralVO> utils = new GdDataHandlerUtils<OtherIntegralVO>(new OtherIntegralVO());
			//插入积分明细表
			msg = utils.save(vo);
			if(msg.isFlag()){
				msg.setMessage("积分操作成功");
			}else{
				msg.setMessage("积分操作失败");
			}
			return msg;
		} catch (DAOException e) {
			e.printStackTrace();
			System.out.println("积分操作失败" + e.getMessage());
			throw new RuntimeException("积分操作失败" + e.getMessage());
		}
//		return new MsgResponse();
	}

	private MsgResponse validateAddIntegral(OtherIntegralVO vo) {
		return null;
	}
}
